(function($) {
	...
	*/
	...
	Registration.prototype.setRoomImage = function(imgLog, size) {
		...
		var bgImg = $('<img class="room-image" src="' + window.WP_Seatreg.uploads_url + '/room_images/' + seatreg.selectedRegistration  + '/' + imgLog + '" />');
	...
	Registration.prototype.buildSkeleton = function() {
		...
		if(this.rooms[this.currentRoom].backgroundImage !== null) {
			var bgImg = $('<img class="room-image" src="' + window.WP_Seatreg.uploads_url + '/room_images/'+ seatreg.selectedRegistration + '/' + this.rooms[this.currentRoom].backgroundImage + '" />');
	...
	*/
	...
	var LAYOUT_EXPORT_VERSION = window.WP_Seatreg.SEATREG_LAYOUT_EXPORT_VERSION;
	...
	function parseLayoutExportFile(text) {
		...
		var roomsAreUsable = layout.roomData.every(function(roomData) {
			return isPlainObject(roomData) &&
				...
				typeof roomData.room.name === 'string' &&
	...
	*/
	...
	function createPriceInput(value, inputEnabled) {
		return $('<input>')
			...
			.attr({ type: 'number', min: '0', oninput: 'this.value = Math.abs(this.value)', value: value })
	...
	var imageSubmitOptions = {
		...
		complete: function(response) {
			 ...
			 if(respObjekt.type == 'ok') {
				 ...
				 var upImgBox = $('<div class="uploaded-image-box"></div>').append('<img src="' + window.WP_Seatreg.uploads_url + '/room_images/' + seatreg.selectedRegistration + '/' + respObjekt.data + '" class="uploaded-image" /> ', addImg, imgRem);
