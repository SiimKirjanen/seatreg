...
function seatreg_remove_all_styles() {
	if( seatreg_is_registration_view_page() ) {
		global $wp_styles;
		$allowedToLoad = array('seatreg-registration-style', 'local-open-sans', 'pg-calendar-style', 'alertify-core', 'alertify-default');
...
function seatreg_public_scripts_and_styles() {
	if ( seatreg_is_registration_view_page() && !empty($_GET['c']) ) {
		...
		wp_enqueue_style('seatreg-registration-style', SEATREG_PLUGIN_FOLDER_URL . 'registration/css/' . $manifest['registration.min.css'] , array(), '1.8.0', 'all');
		...
		wp_enqueue_script('seatreg-registration', SEATREG_PLUGIN_FOLDER_URL . 'registration/js/registration.js' , array('jquery', 'date-format', 'iscroll-zoom', 'jquery-powertip', 'seatreg-utils'), '1.39.0', true);
		...
		$inlineScript .= 'try {';
			...
			$inlineScript .= 'var payPalRestEnabled = "'. esc_js($data->paypal_rest_payments) . '";';
			...
			//Where seatregRoomNouns() and seatregSeatNouns() look first, so the accessors read the same on both sides
			$inlineScript .= 'window.seatreg = window.seatreg || {};';
			$inlineScript .= 'window.seatreg.roomNouns = jQuery.parseJSON(' . wp_json_encode( wp_json_encode( SeatregTerminologyService::getRoomNouns($data) ) ) . ');';
			$inlineScript .= 'window.seatreg.seatNouns = jQuery.parseJSON(' . wp_json_encode( wp_json_encode( SeatregTerminologyService::getSeatNouns($data) ) ) . ');';
	...
	if( seatreg_is_companion_app_page() ) {
		wp_enqueue_script('seatreg-companion-app', SEATREG_PLUGIN_FOLDER_URL . 'companion_app/_expo/static/js/web/AppEntry-2654c845f33ba2d75d1a4d2ba11d0fc8.js' , array(), '1.3.0', true);
