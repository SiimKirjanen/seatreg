...
function seatreg_public_scripts_and_styles() {
	if ( seatreg_is_registration_view_page() && !empty($_GET['c']) ) {
		...
		wp_enqueue_script('seatreg-registration', SEATREG_PLUGIN_FOLDER_URL . 'registration/js/registration.js' , array('jquery', 'date-format', 'iscroll-zoom', 'jquery-powertip', 'seatreg-utils'), '1.32.0', true);
	...
	if( seatreg_is_companion_app_page() ) {
		wp_enqueue_script('seatreg-companion-app', SEATREG_PLUGIN_FOLDER_URL . 'companion_app/_expo/static/js/web/AppEntry-b8c70972178bea8f0189194db4264245.js' , array(), '1.2.0', true);
